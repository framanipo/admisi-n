...
async function checkRegistrados() {
  const config = await getDbConfig();
  const connection = await mysql.createConnection(config);

  try {
    const [columns]: any = await connection.query("SHOW COLUMNS FROM registrados");
    console.log("Columns in registrados:");
    columns.forEach((col: any) => console.log(`- ${col.Field}`));
  } catch (err: any) {
    console.error("Error checking registrados:", err.message);
  } finally {
