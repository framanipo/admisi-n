...
async function checkPreinscripciones() {
  const config = await getDbConfig();
  console.log("Using config:", { ...config, password: '***' });
  const connection = await mysql.createConnection(config);

  try {
    const [columns]: any = await connection.query("SHOW COLUMNS FROM preinscripciones");
    console.log("Columns in preinscripciones:");
    columns.forEach((col: any) => console.log(`- ${col.Field}`));
  } catch (err: any) {
    console.error("Error checking preinscripciones:", err.message);
  } finally {
    await connection.end();
  }
}

checkPreinscripciones();
