...
async function describePreinscripciones() {
  const config = await getDbConfig();
  const connection = await mysql.createConnection(config);

  try {
    const [rows]: any = await connection.query("DESCRIBE preinscripciones");
    console.table(rows);
  } catch (err: any) {
    console.error("Error describing preinscripciones:", err.message);
  } finally {
    await connection.end();
  }
}

describePreinscripciones();
